...
def mse_loss(pred: Tensor, target: Tensor) -> Tensor:
    ...
    if pred._shape != target._shape:
        raise ValueError(
            "Predicted and target tensors must have the same shape for MSE loss."
        )
    ...
    new_tensor = Tensor._create_node(
        data=mse, grad_fn=mse_backward, parents=(pred, target)
    )
...
def mae_loss(pred: Tensor, target: Tensor) -> Tensor:
    ...
    if pred._shape != target._shape:
        raise ValueError(
            "Predicted and target tensors must have the same shape for MAE loss."
        )
    ...
    new_tensor = Tensor._create_node(
        data=mae, grad_fn=mae_backward, parents=(pred, target)
    )
